    ...
    try
    {
       ...
       Invoke-Command -session $session -scriptblock {
        ...
        If ($b -ne $Null)
        {
            ...
            {
                Write-Host "Found $fc folders with $folder of space used. Stuff to clean, maybe?" -ForegroundColor Green
                #######################################################################################################		
                ...
                "$(get-date -format MM/dd/yyy)  *  $(get-date -format HH:mm:ss)   ****   Intial Discovery - There is $foldersize megabytes of space used in the CCMCACHE folder and there are $count items in CACHE." | Add-Content $log
            }
        }
        Else
        {
