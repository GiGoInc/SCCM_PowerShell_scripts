                    ...
                    ForEach-Object -Process {
                        if ( $App = Get-CimInstance -query "Select LocalizedDisplayName,LocalizedDescription from $ObjectClass WHERE ModelName='$_'" @hash | select -Unique  )
                        {
                            ...
                            #     }
                                ...
                                $ADesc = $app.LocalizedDescription
                                $DP+','+$ObjectType+','+$AName+','+$ADesc | Add-Content $Log
                        }    
                    }
                }
                else
                    ...
                    ForEach-Object -Process {
                        if ($Package = Get-CimInstance -query "Select Name from $ObjectClass WHERE PackageID='$_'" @hash )
                        {
                            ...
                            #    }
                                $PKG = $Package.Name 
                                $PKGID = $_ 
                                $DP+','+$ObjectType+','+$PKG+','+$PKGID | Add-Content $Log
                        }
                    }
                }
            }
...
$DPnames = Get-content '.\Get-DPContent--PCList.txt'
    $Log = ".\DP_Content_$DPname.csv"
If (Test-Path $Log){Remove-Item $Log -Force}
ForEach ($DPname in $DPNames)
{
    Write-host 'Checking DP: ' $DPname -ForegroundColor Cyan

    Get-DPContent -DPname $DPname -ObjectType Application
    Get-DPContent -DPname $DPname -ObjectType BootImagePackage
    Get-DPContent -DPname $DPname -ObjectType SoftwareUpdatePackage
    Get-DPContent -DPname $DPname -ObjectType DriverPackage
    Get-DPContent -DPname $DPname -ObjectType ImagePackage
    Get-DPContent -DPname $DPname -ObjectType Package
}
...
write-host "Completed $(Get-Date)"
                                ...
                                $AName = $App.LocalizedDisplayName 
                                $ADesc = $app.LocalizedDescription
                                $DP+','+$ObjectType+','+$AName+','+$ADesc | Add-Content "C:\Temp\DP_Content_$SCCMServer.csv"
