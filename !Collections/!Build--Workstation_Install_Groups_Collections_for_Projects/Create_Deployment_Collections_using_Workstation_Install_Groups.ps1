...
{
    $ParentCollection = Get-WmiObject -Namespace "root\sms\Site_XX1" -Class SMS_Collection -ComputerName 'SERVER' -Filter "Name='$CollectionName'"
    $SubCollection = Get-WmiObject -Namespace "root\sms\Site_XX1" -Class SMS_Collection -ComputerName 'SERVER' -Filter "Name='$InCollectionName'"

    $VerifyDependency = Invoke-WmiMethod -Namespace "root\sms\Site_XX1" -Class SMS_Collection -Name VerifyNoCircularDependencies -ArgumentList @($ParentCollection.__PATH,$SubCollection.__PATH,$null) -ComputerName 'SERVER'
    ...
    If($VerifyDependency){
        ...
        #Create new rule
        $NewRule = ([WMIClass]"\\SERVER\root\SMS\Site_XX1:SMS_CollectionRuleIncludeCollection").CreateInstance()
...
Add-CollectionIncludeRule -CollectionName "WMF 5.1 deployment - Group 01" -InCollectionName "Workstations - Non-RemoteLocale - Group 01"
