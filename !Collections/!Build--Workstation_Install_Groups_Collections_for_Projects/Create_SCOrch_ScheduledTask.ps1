...
Parameter List:
    ...
    /RU  username      Specifies the "run as" user account (user context) under which the task runs.
    ...
    /M    months       Specifies month(s) of the year. Defaults to the first day of the month. 
    ...
    /DU   duration     Specifies the duration to run the task. The time format is HH:mm. 
    ...
    /K                 Terminates the task at the endtime or duration time.
                       This is not applicable for schedule types: ONSTART, ONLOGON, ONIDLE, ONEVENT. Either /ET or /DU must be specified.

    /SD   startdate    Specifies the first date on which the task runs. The format is mm/dd/yyyy. Defaults to the current date. 
    ...
    /DELAY delaytime   Specifies the wait time to delay the running of the task after the trigger is fired.  The time format is mmmm:ss.
...
Examples:
    ...
    ==> Creates a scheduled task "accountant" on the remote machine "ABC" to run calc.exe every five minutes 
	    from the specified start time to end time between the start date and end date.
    ...
    ==> Creates a scheduled task "logtracker" on remote machine "ABC"
        to run notepad.exe every five minutes starting from the specified start time with no end time.
