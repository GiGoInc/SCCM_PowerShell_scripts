...
{
	Write-Output "Pinging...`t$PCName"
    $ping = gwmi Win32_PingStatus -filter "Address='$PCName'"
    if($ping.StatusCode -eq 0)
	{
	    $pcip=$ping.ProtocolAddress;
        Write-Output "Starting SCCM Action - Data Discovery Cycle...."
    Invoke-CMClientDiscoveryDataCycle -Computername $PCName -AsJob
        Start-Sleep -Seconds 2
        Write-Output "Starting SCCM Action - Client Hardware Inventory...."
    Invoke-CMClientHardwareInventory -Computername $PCName  -AsJob
        Write-Output "Starting SCCM Action - Update Deployment Evaluation...."
    Invoke-CMClientUpdateDeploymentEvaluation -Computername $PCName -AsJob
        "$PCName,complete" | Add-Content $Log
	} 
    else
	{
	    Write-Output "$PCName unpingable"
	    "$PCName,unpingable"| Add-Content $Log
	} 
}
